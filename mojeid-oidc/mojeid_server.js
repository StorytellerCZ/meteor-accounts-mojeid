...
OAuth.registerService('mojeid', 2, null, function (query) {
  ...
  if (accessToken) {
    ...
    Object.extend(serviceData, fields);
  }
...
var getToken = function (query) {
  ...
  var serverTokenEndpoint = "https://mojeid.cz/oidc/token/";
  var response;
  ...
  try {
    response = HTTP.post(
      serverTokenEndpoint,
      {
        ...
        params: {
          ...
          redirect_uri: OAuth._redirectUri('mojeid', config),
    ...
    throw Object.extend(new Error(`Failed to get token from MojeID ${serverTokenEndpoint}: ${err.message}`),
      { response: err.response });
  }
  if (response.data.error) {
    // if the http response was a json object with an error attribute
    throw new Error(`Failed to complete handshake with MojeID ${serverTokenEndpoint}: ${response.data.error}`);
  } else {
...
var getConfiguration = function () {
  ...
  if (!config) {
    throw new ServiceConfiguration.ConfigError('Service MojeID not configured.');
  }
  return config;
};
...
MojeID.retrieveCredential = function (credentialToken, credentialSecret) {
  return OAuth.retrieveCredential(credentialToken, credentialSecret);
};

var getUserInfoFromEndpoint = function (accessToken, config, expiresAt) {
  var serverUserinfoEndpoint = 'https://mojeid.cz/oidc/userinfo/';
  var response;
  try {
    ...
    throw Object.extend(new Error(`Failed to fetch userinfo from MojeID ${serverUserinfoEndpoint}: ${err.message}`), { response: err.response });
  }
  if (debug)
    console.debug('XXX: getUserInfo response: ', response.data);
